...
type DashboardData = {
  ...
  weekly: {
    ...
    handledEmails: number;
    agentRuns: number;
    meetingMinutes: number;
  };
  calendarLoad: Array<{ date: string; minutes: number }>;
  nextCommitment: { id: string; title: string; dueAt: string | null } | null;
};
type Email = {
  id: string;
  sender: string;
...
const copy: Record<Kind, [string, string]> = {
  ...
  approvals: [
    "Approvals",
    "Review sensitive actions before Morrow executes them.",
  ],
  activity: [
    "Agent activity",
...
export function WorkspacePage({ kind }: { kind: Kind }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!dataResponse.ok)
        throw new Error("Morrow could not load this workspace view.");
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Morrow could not load this workspace view.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="content">
      <div className="section-head">
        <div>
          <div className="eyebrow">Morrow workspace</div>
...
function Loading() {
  return (
    <section className="card state">
      ...
      <span className="subtle">Morrow is retrieving current data.</span>
    </section>
  );
}
function Failure({
...
function Dashboard({ data }: { data: DashboardData }) {
  const [customizing, setCustomizing] = useState(false),
    [widgets, setWidgets] = useState({ weekly: true, load: true });
  useEffect(() => {
    try {
      const saved = JSON.parse(
        localStorage.getItem("chief-dashboard-widgets") ?? "null",
      ) as typeof widgets | null;
  ...
  const toggleWidget = (key: keyof typeof widgets) =>
    setWidgets((current) => {
      ...
      localStorage.setItem("chief-dashboard-widgets", JSON.stringify(next));
      return next;
    });
  const metrics = [
    {
    ...
    <>
      ...
      <section className="command-hero card">
        <div className="command-copy">
          ...
          <h2>
            {greeting()}, your day is{" "}
            {data.command.dayScore >= 75
              ? "under control"
              : "asking for attention"}
            .
          </h2>
          <p className="subtle">
            {data.command.overdue.length
              ? `${data.command.overdue.length} overdue commitment${data.command.overdue.length === 1 ? "" : "s"} should be handled first.`
              : "No overdue commitments. You have room to work proactively."}
          </p>
        ...
        <div
          ...
          style={
            {
              "--score": `${data.command.dayScore * 3.6}deg`,
            } as React.CSSProperties
          }
        >
          <div>
            <strong>{data.command.dayScore}</strong>
            <span>day score</span>
          </div>
      ...
      <div className="command-grid">
        <section className="card command-panel">
          ...
          {data.command.meetings.length ? (
            data.command.meetings.slice(0, 3).map((meeting) => (
              <div className="command-line" key={meeting.id}>
                <time>
        ...
        <section className="card command-panel focus-panel">
          ...
          <strong className="focus-number">
            {Math.floor(data.command.focusMinutes / 60)}h{" "}
            {data.command.focusMinutes % 60}m
          </strong>
          <span className="subtle">
            Estimated open capacity after today&apos;s synchronized meetings.
          </span>
          <div className="focus-meter">
            <i
              style={{
                width: `${Math.min(100, data.command.focusMinutes / 4.8)}%`,
              }}
            />
          </div>
        </section>
        <section className="card command-panel">
          ...
          {data.command.priorityEmails.length ? (
            data.command.priorityEmails.slice(0, 3).map((email) => (
              <Link
                className="command-line item-link"
                href={`/inbox/${email.id}`}
                key={email.id}
              >
                <span className="dot" />
                <div>
                  ...
                  <span className="subtle">{email.sender}</span>
                </div>
              </Link>
            ))
          ) : (
            <div className="mini-empty">
              No analyzed messages currently need action.
            </div>
          )}
        </section>
      </div>
      {(widgets.weekly || widgets.load) && (
        <div className="delighter-grid">
          {widgets.weekly && (
            <section className="card weekly-recap">
              ...
              <div className="recap-numbers">
                <div>
                  <strong>{data.weekly.completedTasks}</strong>
                  <span>tasks completed</span>
                </div>
                <div>
                  <strong>{data.weekly.handledEmails}</strong>
                  <span>emails handled</span>
                </div>
                <div>
                  <strong>{data.weekly.agentRuns}</strong>
                  <span>agent runs</span>
                </div>
                <div>
                  <strong>
                    {Math.round(data.weekly.meetingMinutes / 60)}h
                  </strong>
                  <span>in meetings</span>
                </div>
              </div>
              <div className="chief-insight">
                <Sparkles size={15} />
                <span>
                  <strong>Morrow noticed:</strong>{" "}
                  {data.weekly.completedTasks > data.weekly.handledEmails
                    ? "You converted more commitments into finished work than inbox processing this week."
                    : "Inbox processing led your momentum this week; protect a focus block for the next commitment."}
                </span>
              </div>
            </section>
          )}
          {widgets.load && (
            <section className="card calendar-heatmap">
              <div className="section-head">
                <div>
                  <div className="eyebrow">Meeting load</div>
                  <h2>Two-week rhythm</h2>
                </div>
                <CalendarDays size={18} />
              </div>
              <div className="heatmap-row">
                {data.calendarLoad.map((day) => (
                  <div
                    key={day.date}
                    title={`${new Date(day.date).toLocaleDateString()}: ${day.minutes} meeting minutes`}
                  >
                    <i
                      style={{
                        opacity: Math.max(0.1, Math.min(1, day.minutes / 240)),
                      }}
                    />
                    <span>
              ...
              {data.nextCommitment ? (
                <div className="next-commitment">
                  ...
                  <strong>{data.nextCommitment.title}</strong>
                  <Link href="/tasks">
                    Open task <ArrowUpRight size={13} />
                  </Link>
                </div>
              ) : (
                <div className="mini-empty">No open commitment is waiting.</div>
              )}
            </section>
          )}
        </div>
...
function Inbox({ data }: { data: Email[] }) {
  const [emails, setEmails] = useState(data),
    ...
    [notice, setNotice] = useState<{ text: string; ids?: string[] } | null>(
      null,
    );
  const active = emails[cursor];
  const apply = useCallback(
    async (action: "SNOOZE" | "HANDLE", override?: string[]) => {
      const ids =
        override ?? (selected.size ? [...selected] : active ? [active.id] : []);
      ...
      if (response.ok) {
        setEmails((current) =>
          current.filter((email) => !ids.includes(email.id)),
        );
        setSelected(new Set());
        setCursor(0);
        setNotice({
  ...
  const undo = async () => {
    if (!notice?.ids) return;
    const response = await fetch("/api/inbox/triage", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ids: notice.ids, action: "RESTORE" }),
    });
    if (response.ok) location.reload();
  };
  const run = useCallback(
    async (action: "ANALYZE" | "TASK" | "REPLY") => {
      if (!active) return;
      if (action === "REPLY") {
        location.assign(`/inbox/${active.id}?compose=1`);
        return;
      }
      const response = await fetch(
        `/api/inbox/${active.id}/${action === "ANALYZE" ? "analyze" : "task"}`,
        { method: "POST" },
      );
      setNotice({
        text: response.ok
  ...
  useEffect(() => {
    const keyboard = (event: KeyboardEvent) => {
      ...
      if (key === "j") {
        ...
        setCursor((current) => Math.min(emails.length - 1, current + 1));
      }
      if (key === "k") {
        event.preventDefault();
        setCursor((current) => Math.max(0, current - 1));
      }
      if (key === "a") void run("ANALYZE");
      if (key === "t") void run("TASK");
      if (key === "r") void run("REPLY");
      if (key === "s") void apply("SNOOZE");
      if (key === "e") void apply("HANDLE");
      if (key === "enter" && active) location.assign(`/inbox/${active.id}`);
      if (key === "escape") setSelected(new Set());
    };
    window.addEventListener("keydown", keyboard);
    return () => window.removeEventListener("keydown", keyboard);
  }, [active, apply, emails.length, run]);
      ...
      <section className="card">
        <div className="triage-toolbar">
          <div>
            <Keyboard size={16} />
            <strong>
              {selected.size ? `${selected.size} selected` : "Keyboard triage"}
            </strong>
            <span className="subtle">
              J/K navigate · A analyze · T task · R reply · S snooze · E handled
            </span>
          </div>
          {selected.size > 0 && (
            <div className="row">
              <button className="button" onClick={() => void apply("SNOOZE")}>
                <Clock3 size={14} />
                Snooze
              </button>
              <button className="button" onClick={() => void apply("HANDLE")}>
                <Archive size={14} />
                Handled
              </button>
            </div>
        ...
        {notice && (
          <div className="triage-notice">
            <span>{notice.text}</span>
            {notice.ids && <button onClick={() => void undo()}>Undo</button>}
          </div>
        )}
        {emails.length ? (
          <div className="list">
            {emails.map((email, index) => (
              <div
                className={`triage-row ${cursor === index ? "cursor" : ""} ${selected.has(email.id) ? "selected" : ""}`}
                key={email.id}
              >
                <button
                  className="select-email"
                  onClick={() => toggle(email.id)}
                  aria-label={`${selected.has(email.id) ? "Deselect" : "Select"} ${email.subject}`}
                >
                  <i>{selected.has(email.id) && <CheckCircle2 size={13} />}</i>
                </button>
                <Link
                  className="list-item item-link"
                  href={`/inbox/${email.id}`}
...
function PushSyncStatus() {
  ...
  const start = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Live Gmail updates could not be enabled.");
      await load();
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "Live Gmail updates could not be enabled.",
      );
    } finally {
      setStarting(false);
    }
  ...
  return (
    <section className={`push-status ${status.active ? "active" : ""}`}>
      ...
      <div className="grow">
        <strong>
          {status.active
            ? "Live Gmail updates are active"
            : status.configured
              ? "Enable live Gmail updates"
              : "Live updates need Pub/Sub setup"}
        </strong>
        <span className="subtle">
          {status.active
            ? `Morrow listens for new inbox activity${status.lastNotificationAt ? ` · Last signal ${formatDate(status.lastNotificationAt)}` : ""}.`
            : status.configured
              ? "Start a secure Gmail watch to sync without polling."
              : "Add the Google Cloud project, Pub/Sub topic, OIDC audience, and service account settings."}
        </span>
      ...
      {status.active && status.expiresAt && (
        <span className="badge">
          Renews before {new Date(status.expiresAt).toLocaleDateString()}
        </span>
      )}
    </section>
  );
...
function Tasks({ data }: { data: Task[] }) {
  const columns = [
    ...
    [view, setView] = useState<"kanban" | "sprint" | "list" | "matrix">(
      "kanban",
    );
  useEffect(() => {
    ...
    <>
      ...
      <div className="task-toolbar">
        <div>
          ...
          <span className="subtle">
            Switch perspective without changing your data.
          </span>
        </div>
        ...
        <div className="card task-list-view">
          ...
          {!tasks.length && (
            <Empty
              Icon={ListTodo}
              title="No tasks yet"
              detail="Tasks created from selected emails will appear here."
            />
          )}
        </div>
      )}
      {view === "sprint" && (
        <ScrumWorkspace
          overview={<SprintView tasks={tasks} columns={columns} move={move} />}
        />
      )}
      {view === "matrix" && (
        <MatrixView tasks={tasks} columns={columns} move={move} />
      )}
    </>
  );
}
function TaskCard({
  ...
  task: Task;
  columns: readonly ["BACKLOG", "TODO", "IN_PROGRESS", "WAITING", "DONE"];
  move: (task: Task, status: (typeof columns)[number]) => Promise<void>;
  compact?: boolean;
}) {
  return (
    <div className={`task ${compact ? "task-row" : ""}`}>
      <div className="grow">
        ...
        <div className="row task-meta">
          ...
          {task.dueAt && (
            <span className="subtle">Due {formatDate(task.dueAt)}</span>
          )}
        </div>
      </div>
      <label className="field task-status">
        ...
        <select
          ...
          value={task.status}
          onChange={(event) =>
            void move(task, event.target.value as (typeof columns)[number])
          }
        >
          {columns.map((option) => (
...
function SprintView({
  ...
  const completed = tasks.filter((task) => task.status === "DONE").length,
    ...
    groups = [
      ...
      {
        title: "Completed",
        detail: "Finished commitments",
        items: tasks.filter((task) => task.status === "DONE"),
      },
    ];
  return (
    <div className="sprint-view">
      <section className="card sprint-health">
        <div>
          <div className="eyebrow">Sprint health</div>
          <strong>{progress}% complete</strong>
          <span className="subtle">
            {completed} of {tasks.length} commitments finished
          </span>
        </div>
        <div
      ...
      <div className="sprint-groups">
        {groups.map((group) => (
          <section className="card" key={group.title}>
            <div className="section-head">
              <div>
                <h2>{group.title}</h2>
                <span className="subtle">{group.detail}</span>
              </div>
            ...
            {!group.items.length && (
              <div className="subtle column-empty">Nothing here yet</div>
            )}
          </section>
        ))}
      </div>
...
function MatrixView({
  ...
  const open = tasks.filter((task) => task.status !== "DONE"),
    quadrants = [
      {
        title: "Do now",
        detail: "Urgent or high priority",
        items: open.filter(
          (task) =>
      ...
      {
        title: "Plan next",
        detail: "Important, not urgent",
        items: open.filter(
          (task) => task.priority === "MEDIUM" && task.status !== "WAITING",
        ),
      },
      {
        title: "Waiting",
        detail: "Blocked or delegated",
        items: open.filter((task) => task.status === "WAITING"),
      },
      {
        title: "Low leverage",
        detail: "Review when capacity opens",
        items: open.filter(
          (task) => task.priority === "LOW" && task.status !== "WAITING",
        ),
      },
    ];
  return (
    <div className="matrix-view">
      {quadrants.map((quadrant, index) => (
        <section
          className={`card matrix-quadrant q${index + 1}`}
          key={quadrant.title}
        >
          <div className="section-head">
            <div>
              <h2>{quadrant.title}</h2>
              <span className="subtle">{quadrant.detail}</span>
            </div>
            <span className="badge">{quadrant.items.length}</span>
          </div>
          ...
          {!quadrant.items.length && (
            <div className="subtle column-empty">No matching tasks</div>
          )}
        </section>
      ))}
    </div>
...
function FocusPlanner() {
  ...
  useEffect(() => {
    fetch("/api/calendar/focus")
      .then((response) => (response.ok ? response.json() : []))
      .then(setSuggestions)
      .finally(() => setLoading(false));
  }, []);
  const propose = async (slot: { start: string; end: string }) => {
    setPending(slot.start);
    const response = await fetch("/api/calendar/focus", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        title: "Focus time",
    ...
    setMessage(
      response.ok
        ? "Focus block sent to Approvals."
        : "Focus block could not be proposed.",
    );
  };
  return (
    <section className="focus-planner card">
      <div className="section-head">
        <div>
          <div className="eyebrow">Schedule defense</div>
          <h2>Protect deep work</h2>
          <span className="subtle">
            Open 90-minute windows calculated from your live calendar.
          </span>
        </div>
        <ShieldCheck size={18} />
      </div>
      {loading ? (
        ...
        <div className="focus-suggestions">
          {suggestions.map((slot) => (
            <button
              ...
              onClick={() => void propose(slot)}
              disabled={pending === slot.start}
              key={slot.start}
            >
        ...
        <div className="mini-empty">
          No 90-minute focus windows are available in the next week.
        </div>
      )}
    ...
    <section className="card">
      {data.length ? (
        ...
        <Empty
          ...
          detail="Sensitive actions generated by Morrow will wait here."
        />
      )}
    </section>
  );
    ...
    <section className="card">
      {data.length ? (
        ...
        <Empty
          ...
          detail="Operational events will appear after Morrow processes work."
        />
      )}
    </section>
  );
  ...
  return (
    <div className="settings-layout">
      <section className="card settings-span">
        <div className="section-head">
          <div>
            ...
            <div className="subtle">Shape how Morrow looks and moves.</div>
          </div>
          <Palette size={18} />
        </div>
          ...
          <PreferenceGroup label="Motion">
            {(["full", "reduced"] as const).map((value) => (
              <Choice
                ...
                onClick={() =>
                  setForm((current) => ({ ...current, motion: value }))
                }
              />
            ))}
          </PreferenceGroup>
          <PreferenceGroup label="Density">
            {(["comfortable", "compact"] as const).map((value) => (
              <Choice
                ...
                onClick={() =>
                  setForm((current) => ({ ...current, density: value }))
                }
              />
            ))}
          </PreferenceGroup>
      ...
      <section className="card settings-span">
        <div className="section-head">
          <div>
            ...
            <div className="subtle">
              Choose the default planning perspective.
            </div>
          </div>
          <Layers3 size={18} />
        </div>
        <div className="view-choices">
          {(
            ...
            <button
              ...
              onClick={() =>
                setForm((current) => ({ ...current, taskView: value }))
              }
              key={value}
            >
              <ListTodo size={18} />
      ...
      <div className="settings-savebar settings-span">
        <div>
          <strong>
            {message ?? "Preferences are saved to your workspace."}
          </strong>
          <span className="subtle">
            Changes apply across signed-in sessions.
          </span>
        </div>
        <button
          className="button primary"
